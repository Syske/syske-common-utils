package io.github.syske.commont.utils.http;

import io.github.syske.commont.utils.collection.CollectionUtils;
...
public class HttpClientUtil {
    ...
    private final static String APPLICATION_JSON_CHARSET = "application/json;charset=utf-8";
    ...
    public Map<String, String> post(String url) {
        ...
        if (!CollectionUtils.isEmpty(nvps)) {
            try {
                httpPost.setEntity(new UrlEncodedFormEntity(nvps, UTF8_CHARSET));
        ...
        if (!StringUtils.isEmpty(requestParam)) {
            try {
                httpPost.setEntity(new StringEntity(requestParam, UTF8_CHARSET));
        ...
        if (!CollectionUtils.isEmpty(nvps)) {
            for (NameValuePair nvp : nvps) {
                ...
                if (!StringUtils.isEmpty(value)) {
                    builder.addTextBody(nvp.getName(), value, ContentType.create("text/plain", UTF8_CHARSET));
                }
            }
        }
    ...
    private Map<String, String> getHttpResult(CloseableHttpResponse response, String url, HttpUriRequest request, Map<String, String> resultMap) {
        ...
        if (entity != null) {
            try {
                result = EntityUtils.toString(entity, UTF8_CHARSET);
    ...
    public static String form(String url, Map<String, String> params) {
        ...
        try {
            ...
            OutputStreamWriter osw = new OutputStreamWriter(con.getOutputStream(), UTF8_CHARSET);
        ...
        try {
            ...
            BufferedReader br = new BufferedReader(new InputStreamReader(con.getInputStream(), UTF8_CHARSET));
            String temp;
    ...
    public static String encodeParams(String... params){
        return encodeParams(Charset.forName(UTF8_CHARSET), params);
    }
    ...
    public String doPost(String url, String paramString, Map<String, String> headerMap) throws Exception {
        ...
        StringEntity entity = new StringEntity(paramString, UTF8_CHARSET);

        // post请求是将参数放在请求体里面传过去的;这里将entity放入post请求体中
        httpPost.setEntity(entity);

        httpPost.setHeader("Content-Type", APPLICATION_JSON_CHARSET);
        if(headerMap != null) {
        ...
        try {
            ...
            if (responseEntity != null) {
                responseContent = EntityUtils.toString(responseEntity, UTF8_CHARSET);
    ...
    /**
     * POST请求 默认是 UTF-8 编码，请求类型默认"application/json"
     * post("http://xxx.com/search", "a=bcd", "b=xxx");
     * @param url
     * @param params 请单个编写，内部会自动将等号后字符编码
     * @return
    ...
    public static String post(String url, String... params){
        return post(url, Charset.forName(UTF8_CHARSET), APPLICATION_JSON_CHARSET, params);
    }
     ...
     * @param url
     * @param charset 字符编码
     * @param contentType 媒体类型，比如"application/json"
     * @param params 请单个编写，内部会自动将等号后字符编码
     * @return
     */
    public static String post(String url, Charset charset, String contentType, String... params) {
        PrintWriter out = null;
        InputStream in = null;
        String result = "";
        try {
            ...
            conn.setRequestProperty("user-agent", "Mozilla/4.0 (compatible; MSIE 6.0; Windows NT 5.1; SV1)");
            conn.setRequestProperty("Accept-Charset", UTF8_CHARSET);
            conn.setConnectTimeout(60000);
            conn.setReadTimeout(300000);
